...
4. **Read the authoritative redirect URI** from `/data/callbacks/<plugin-name>/ready.json` and register that *exact* string in the OAuth client's **Authorized redirect URIs** (the field you left empty in Step 2). Never construct this value yourself from the plugin's name — a scoped install has a different effective name than the plugin's base name, and Google matches the redirect URI byte-for-byte. Only the value in `ready.json` is authoritative.
