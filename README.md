...
| Tool | Description |
|---|---|
| `setup_gmail` | Casa's declared setup tool (`casa.setupTool`), auto-dispatched once the consent DM is approved. Argument-free and idempotent: mints an authorization link only when one is actually needed, and otherwise reports `already_connected` (verified live, not assumed), `already_pending` (a valid link is already out — it will not issue a second), or `retry_later` (the connection could not be checked just now; nothing changed). A revoked credential yields a fresh recovery link rather than a false "already connected". Not a protected tool — casa dispatches it unprompted, so an approval prompt would deadlock the setup episode |
